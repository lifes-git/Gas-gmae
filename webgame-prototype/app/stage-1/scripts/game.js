(function () {
  ...
  var kitchenScenes = window.createKitchenScenes({
    ...
    complete: function (id) {
      ...
      elements.guideMascot.src = "assets/common/mascots/mascot-somyeongi-success-logo-v1.svg";
  ...
  function setMissionStatus(kind, message) {
    ...
    if (!kind) {
      elements.missionMascot.src = "assets/common/mascots/mascot-somyeongi-question-logo-v1.svg";
      return;
    }
    elements.missionSpeech.classList.add("is-" + kind);
    elements.missionMascot.src = kind === "success"
      ? "assets/common/mascots/mascot-somyeongi-success-logo-v1.svg"
      : "assets/common/mascots/mascot-somyeongi-caution-logo-v1.svg";
    playFeedback(kind);
  }
  ...
  function renderProgress() {
    var solvedCount = state.solved.size;
    var count = state.collected.size;
    ...
    var complete = solvedCount === HAZARDS.length;
  ...
  function renderMissionVisual(visualName) {
    ...
    mascot.src = visualName === "towel"
      ...
      : visualName === "butane-outdoor"
        ? "assets/common/mascots/mascot-somyeongi-success-logo-v1.svg"
        : "assets/common/mascots/mascot-somyeongi-question-logo-v1.svg";
    mascot.alt = "";
    ...
    if (visualName === "towel") {
      ...
      basket.src = "assets/stage-1/props/prop-basket-empty-v1.png";
      basket.alt = "";
    ...
    if (visualName === "butane-outdoor") {
      ...
      disposalBin.innerHTML = '<img src="assets/stage-1/props/prop-can-recycling-bin-front-v2.png" alt="">';
  ...
  function resetGame() {
    ...
    elements.guide.textContent = dialogue.intro;
    elements.guideMascot.src = "assets/common/mascots/mascot-somyeongi-guide-logo-v1.svg";
    document.querySelectorAll(".is-solved").forEach(function (item) { item.classList.remove("is-solved"); });
    if (window.ProgressCollection) window.ProgressCollection.reset(elements.progressDots);
    renderProgress();
    window.clearTimeout(state.nextPromptTimer);
  ...
  var pendingProgressCollection = null;
  elements.safetyRuleConfirm.addEventListener("click", function () {
    playFeedback("tap");
    if (elements.safetyRuleDialog.dataset.stage !== "1") return;
    var hazard = elements.safetyRuleDialog.dataset.hazard;
    var rule = window.SAFETY_RULE_CARDS[hazard];
    if (!rule || state.collected.has(hazard)) return;
    pendingProgressCollection = {
      item:hazard,
      icon:rule.progressIcon || rule.icon,
      label:rule.label,
      sourceRect:elements.safetyRuleIcon.getBoundingClientRect()
    };
  });
  elements.safetyRuleDialog.addEventListener("cancel", function (event) { event.preventDefault(); });
  elements.safetyRuleDialog.addEventListener("close", function () {
    if (elements.safetyRuleDialog.dataset.stage !== "1") return;
    if (pendingProgressCollection && window.ProgressCollection) {
      var collection = pendingProgressCollection;
      pendingProgressCollection = null;
      if (elements.dialog.open) elements.dialog.close();
      document.querySelectorAll(".kitchen-detail-dialog[open]").forEach(function (dialog) { dialog.close(); });
      window.ProgressCollection.collect({
        container:elements.progressDots,
        item:collection.item,
        icon:collection.icon,
        label:collection.label,
        sourceRect:collection.sourceRect
      }).then(function () {
        state.collected.add(collection.item);
        renderProgress();
        playFeedback("move");
