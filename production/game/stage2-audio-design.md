...
| 이벤트 | 사운드 ID | 표현 | 길이/의도 |
|---|---|---|---|
| Stage 1→2 장면 전환 | `outingDoorShut`, `outingFootsteps` | 실제 문 닫힘 뒤 기존 발걸음 3회 | `며칠 후…` 구간은 비우고 노을 복귀 화면이 드러날 때 Stage 2 BGM을 페이드인 |
...
| 번호 입력 | `phoneDial` | 실제 전화 키패드 DTMF 4회 입력음 | 실제 3×4 번호 패드 클로즈업과 동기화 |
